...
locals {
  ...
  managed_identity_app_settings = var.user_assigned_identity_client_id != null ? {
    "Acmebot__ManagedIdentityClientId" = var.user_assigned_identity_client_id
  } : {}

  auth_app_settings = var.enable_auth ? {
    "MICROSOFT_PROVIDER_AUTHENTICATION_SECRET" = azuread_application_password.acmebot[0].value
  } : {}
}
